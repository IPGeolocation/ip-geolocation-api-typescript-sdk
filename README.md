# IPGeolocation TypeScript SDK

Official TypeScript SDK for the IPGeolocation IP Location API.

Look up IPv4, IPv6, and domains with `/v3/ipgeo` and `/v3/ipgeo-bulk`. Get geolocation, company, ASN, timezone, network, hostname, abuse, user-agent, and security data from one API. Includes typed request and response models, ESM, CommonJS, and declaration files.
...
You can pass plain request objects, which is the most common option in TypeScript code. If you want request validation before the call is sent, use `LookupIpGeolocationRequest` and `BulkLookupIpGeolocationRequest`.
...
const request: LookupIpGeolocationRequestInit = {
  ip: "8.8.8.8",
  fields: ["location.country_name", "time_zone.name"],
};

const response = await client.lookupIpGeolocation(request);

console.log(response.data.location?.countryName);
console.log(response.data.timeZone?.name);
```
...
If you want validation before sending the call, create the request explicitly.
...
Each bulk result is either a success with `data` or an error with `error.message`. TypeScript narrows it automatically on `result.success`.
